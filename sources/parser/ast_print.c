...
/*   Updated: 2025/03/02 14:42:29 by thomarna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_cmd_args(t_cmd_expr *cmd, int indent)
{
	...
	{
		printf("%*s└─ ARG[%d]: %s'%s'%s\n", indent + 2, "", i, COLOR_CYAN,
			cmd->argv[i], COLOR_RESET);
	}
}
...
static void	ast_print_recursive(t_ast *node, int indent)
{
	...
	case AST_TOKEN:
		if (node->expr.token)
			printf(" (%s'%s'%s)", COLOR_CYAN, node->expr.token->val,
				COLOR_RESET);
		break ;
	case AST_CMD:
		if (node->expr.cmd.argc > 0)
			printf(" [%s%s%s]", COLOR_CYAN, node->expr.cmd.argv[0],
				COLOR_RESET);
		break ;
	case AST_REDIR_IN:
	case AST_REDIR_OUT:
	case AST_REDIR_APP:
		if (node->expr.redir.file)
			printf(" -> %s'%s'%s", COLOR_CYAN, node->expr.redir.file,
				COLOR_RESET);
		break ;
	default:
		break ;
